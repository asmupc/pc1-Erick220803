	...
	bool _eliminar(Nodo<T>*& nodo, T e) {
		if (nodo == nullptr) return false;
			...
			if (r < 0) {
				...
				if (nodo->der == nullptr && nodo->izq == nullptr) {//caso 1 nodo hoja
					nodo = nullptr;
					delete nodo;
					return true;
				}
				else if (nodo->izq == nullptr) { //caso 2 nodo con hijo derecho
					nodo = nodo->der;
					return true;
				}
				else if (nodo->der == nullptr) { //caso 3 nodo con hijo izquierdo
					nodo = nodo->izq;
					return true;
				}
				else { //caso 4
...
public:
	ArbolBB(void(*otroPunteroAFuncion)(T)) {
		...
		this->comparar = [](T a, T b)->int {/*return a - b;*/if (a > b)return 1; else if (a < b)return -1; else return 0; };
		raiz = nullptr;
	}
	bool insertar(T e) {
